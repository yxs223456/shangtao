...
use wstmart\admin\validate\Shops as Validate;
use think\Loader;
...
class Shops extends Base
{
    ...
    public function handleApply()
    {
        ...
        if (!$validate->scene('handleApply')->check(Input('post.'))) return WSTReturn($validate->getError());
        //判断经营范围
        ...
        try {
            ...
            */
            ...
            if ($shops->applyStatus != $data['applyStatus']) $this->sendMessages($shopId, $data, 'handleApply');
            Db::commit();
...
//	}
    ...
    public function sendMessages($shopId, $data, $method)
    {
        $shops = model('shops')->get($shopId);
        if ((int)$data['applyStatus'] == 2) {
            //如果存在手机则发送手机号码提示
...
//            }
            ...
            if ((int)WSTConf('CONF.wxenabled') == 1) {
                ...
                $params['NOW_TIME'] = date('Y-m-d H:i:s');
                $params['REASON'] = "申请入驻成功";
                WSTWxMessage(['CODE' => 'WX_SHOP_OPEN_SUCCESS', 'shopId' => $shopId, 'params' => $params]);
            }
        } else {
            //如果存在手机则发送手机号码提示
            $tpl = WSTMsgTemplates('PHONE_SHOP_OPEN_FAIL');
...
//            }
            ...
            if ((int)WSTConf('CONF.wxenabled') == 1) {
                ...
                $params['NOW_TIME'] = date('Y-m-d H:i:s');
                $params['REASON'] = $data['applyDesc'];
                WSTWxMessage(['CODE' => 'WX_SHOP_OPEN_FAIL', 'shopId' => $shopId, 'params' => $params]);
            }
        }
    }
...
//	}
    ...
    public function add()
    {
        ...
        try {
            ...
            $data['applyStatus'] = 2;
            $data['applyDesc'] = '';
            $this->sendMessages($shopId, $data, 'add');

            $this->addPingUser($shopId);
     ...
     * @throws Exception
     */
    private function addPingUser($shopId)
    {
        $data = input("post.");
        $settleType = $data["settleType"];
        if (empty($settleType)) {
            throw new Exception("结算方式数据错误");
        }
        ...
        $recipient = array();
        // 创建结算账户
        if ($settleType == PingSettles::SETTLE_TYPE_BANK) {
            $recipient["account"] = $data["bankNo"] ?? "";
            $recipient["name"] = $data["bankUserName"] ?? "";
            $recipient["type"] = $data["transferType"] ?? "";
            $recipient["open_bank_code"] = $data["openBankCode"] ?? "";
        } else if ($settleType == PingSettles::SETTLE_TYPE_ALI) {
            $recipient["account"] = $data["aliAccount"] ?? "";
            $recipient["name"] = $data["aliName"] ?? "";
            $recipient["type"] = $data["aliTransferType"] ?? "";
            $recipient["account_type"] = $data["accountType"] ?? "";
        } else if ($settleType == PingSettles::SETTLE_TYPE_WX) {
            $recipient["account"] = $data["wxOpenId"] ?? "";
        } else if ($settleType == PingSettles::SETTLE_TYPE_WX_JSAPI) {
            $recipient["account"] = $data["wxJsapiOpenId"] ?? "";
        } else if ($settleType == PingSettles::SETTLE_TYPE_WX_LITE) {
            $recipient["account"] = $data["wxLiteOpenId"] ?? "";
        }

//        $channel = PingSettles::getChannel($settleType);
//        $settle = Ping::settleAccountCreate($pingUserId, $channel, $recipient);
        ...
        $insertData = array(
            ...
            'openBankCode' => $recipient["open_bank_code"] ?? "",
//            'settleJson' => json_encode($settle->jsonSerialize())
            'settleJson' => json_encode(array())
        );

        $settleId = Db::name('ping_settles')->insertGetId($insertData);
        return true;
    }
     ...
     * @throws Exception
     */
    private function editPingUser($shopId)
    {
        $data = input("post.");
        $settleType = $data["settleType"];
        // 没有结算方式数据，不需要更新
        ...
        $pingUser = Db::name("ping_users")->where("shopId", $shopId)->find();
        $pingUserId = empty($pingUser["pingUserId"]) ? "" : $pingUser["pingUserId"];
        if (empty($pingUserId)) {
            throw new Exception("ping用户不存在,请先添加用户");
        }
        $channel = PingSettles::getChannel($settleType);
//        $settle = Ping::settleAccountCreate($pingUserId, $channel, $recipient);
...
//            'settleJson' => json_encode($settle->jsonSerialize())
        ...
        $isExists = Db::name('ping_settles')
            ->where("shopId", $shopId)
            ->where("settleType", $settleType)
            ->find();
        if (empty($isExists)) {
            return Db::name('ping_settles')->insertGetId($insertData);
        }
