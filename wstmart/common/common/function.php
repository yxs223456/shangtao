...
//检测商户管理员手机号是否已注册
function WSTCheckShopAdminPhoneExists($phone){
    if($phone=='')return WSTReturn("登录账号不能为空");
    $rs = Db::name("shop_admin")->where("userPhone", $phone)->count();
    if($rs==0){
        return WSTReturn("该登录账号可用",1);
    }
    return WSTReturn("对不起，登录账号已存在");
...
function WSTUploadPic($fromType=0){
    ...
    $info = $file->validate($rule)->rule('uniqid')->move(\think\facade\Env::get('root_path').'upload/'.$dir."/".date('Y-m'));
    if($info){
        $filePath = $info->getPathname();
        $filePath = str_replace(\think\facade\Env::get('root_path'),'',$filePath);
...
function WSTUserPhoto($userPhoto=''){
	if(substr($userPhoto,0,4)!='http' && $userPhoto){
		$userPhoto  = Request::root().'/'.$userPhoto;
	}else if(!$userPhoto){
		$userPhoto  = Request::root().'/'.WSTConf('CONF.userLogo');
	}
	return $userPhoto;
}
