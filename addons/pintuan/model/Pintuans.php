...
class Pintuans extends Base{
	...
	public function unSaleRefund($tuanId){
		$pusers = Db::name('pintuan_users pu')->join("__USERS__ u","pu.userId=u.userId")
				...
				->where(["tuanStatus"=>1,"tuanId"=>$tuanId])
				->field("pu.id,pu.tuanId,p.goodsName,u.userId,u.userName,u.loginName,u.userPhoto,pu.isHead,pu.tuanNo,pu.createTime,pu.realTotalMoney,pu.orderNo,pu.payType,pu.payFrom")
				->select();
		for($i=0,$j=count($pusers);$i<$j;$i++){
			$puser = $pusers[$i];
			if($puser['payType']==1 && $puser['payFrom']=='wallets' && $puser['payFrom']=='others'){
	        	$rs = $this->saveTuanRefund($puser);
	        }else if($puser['payType']==1 && $puser['payFrom']=='weixinpays'){
	...
	public function pageQuery(){
		...
		$page = Db::name('pintuans')->alias('p')->join('__GOODS__ g','p.goodsId=g.goodsId','inner')
		          ...
		          ->where($where)
		          ->field('g.goodsName,g.goodsImg,g.marketPrice,p.*')
	...
	public function getTuanInfo($tuanNo){
		$userId = (int)session('WST_USER.userId');
		$tuan = $this->alias('p')->join('__PINTUAN_USERS__ pu','p.tuanId=pu.tuanId')
            		 ->where(["pu.tuanNo"=>$tuanNo,"pu.isHead"=>1])
            		 ->field("pu.tuanNo,pu.tuanStatus,p.tuanId,p.tuanTime,p.tuanNum,p.goodsId,p.goodsName,p.orderNum,p.tuanPrice,p.goodsImg,pu.createTime")
            		 ->find();
        $cdate = $tuan['createTime'];
     	...
     	$goods = Db::name('goods')->field("goodsId,goodsName,shopPrice")->where(['goodsId'=>$tuan["goodsId"],'dataFlag'=>1])->find();
	...
	*/
	...
	public function tuanRefund(){
		$pusers = Db::name('pintuan_users pu')->join("__USERS__ u","pu.userId=u.userId")
				...
				->field("pu.id,pu.tuanId,u.userId,u.userName,u.loginName,u.userPhoto,pu.isHead,pu.tuanNo,pu.createTime,pu.realTotalMoney,pu.orderNo,pu.payType,pu.payFrom")
				->select();
		for($i=0,$j=count($pusers);$i<$j;$i++){
			Db::startTrans();
			try{
