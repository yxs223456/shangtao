...
class Groupons extends Base{
	...
	public function addMobileBtn(){
		...
		$data["btnName"] = "团购活动";
		$data["btnSrc"] = 0;
		$data["btnUrl"] = "/addon/groupon-goods-molists";
		...
		$data["btnName"] = "团购活动";
		$data["btnSrc"] = 1;
		$data["btnUrl"] = "/addon/groupon-goods-wxlists";
	...
	public function pageQueryByShop(){
		...
		$where = ['g.shopId'=>$shopId,'gu.dataFlag'=>1,'g.dataFlag'=>1,'g.isSale'=>1];
	...
	public function submitByEntity($carts,$orderSrc = 0,$uId=0){
        ...
        //再次检查团购是否已满
        $goods = $carts['carts']['goods'];
        $tmpGroupon = $this->where(['grouponId'=>$carts['carts']['grouponId']])->lock(true)->field('grouponNum')->find();
		if($tmpGroupon['grouponNum']<$goods['cartNum'])return WSTReturn("下单失败，团购商品库存不足");
